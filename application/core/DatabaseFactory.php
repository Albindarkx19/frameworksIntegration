...
class DatabaseFactory
{
    ...
    public function getMysqliConnection() {
        if (!$this->mysqli_database) {

            /**
             * Check DB connection in try/catch block. Also when MySQLi is not constructed properly,
             * prevent to exposing database host, username and password in plain text
             * by throwing custom error message
             */
            try {
                mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);
                $this->mysqli_database = new mysqli(
                   Config::get('DB_HOST'),
                   Config::get('DB_USER'),
                   Config::get('DB_PASS'),
                   Config::get('DB_NAME'),
                ...
                $this->mysqli_database->set_charset(Config::get('DB_CHARSET'));
