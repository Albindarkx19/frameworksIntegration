<div class="chat-container">
    <div class="chat-sidebar">
        ...
        <div class="chat-user-buttons">
            ...
            <a href="<?= Config::get('URL') . 'message/groupConversation/' . $this->group->group_id; ?>"
               class="user-chat-button active group-button">
                <span class="user-button-icon">👥</span>
                <span class="user-button-name"><?= htmlspecialchars($this->group->group_name); ?></span>
            </a>

            <!-- Other Groups -->
            <?php if (!empty($this->groups)) { ?>
                <?php foreach ($this->groups as $g) { ?>
                    <?php if ($g->group_id == $this->group->group_id) continue; ?>
                    <a href="<?= Config::get('URL') . 'message/groupConversation/' . $g->group_id; ?>"
                       class="user-chat-button group-button">
                        <span class="user-button-icon">👥</span>
                        <span class="user-button-name"><?= htmlspecialchars($g->group_name); ?></span>
                        <?php if ($g->unread_count > 0) { ?>
                            <span class="user-unread-badge"><?= $g->unread_count; ?></span>
                        <?php } ?>
                    </a>
                <?php } ?>
            <?php } ?>
            ...
            <?php if (!empty($this->conversations)) { ?>
                <div class="sidebar-divider">Direct Messages</div>
            <?php } ?>
    ...
    <div class="chat-main">
        ...
        <div class="chat-header">
            <div class="chat-header-user">
                <div class="chat-header-avatar group-header-avatar">
                    <span class="group-icon-large">👥</span>
                </div>
                <div class="chat-header-info">
                    <h3><?= htmlspecialchars($this->group->group_name); ?></h3>
                    <span class="chat-header-email">
                        <?= count($this->members); ?> members:
                        <?php
                        $member_names = array_map(function($m) { return $m->user_name; }, $this->members);
                        echo htmlspecialchars(implode(', ', array_slice($member_names, 0, 3)));
                        if (count($member_names) > 3) {
                            echo ' +' . (count($member_names) - 3);
                        }
                        ?>
                    </span>
                </div>
        ...
        <div class="chat-input-area">
            <form action="<?= Config::get('URL'); ?>message/sendToGroup" method="post" id="chatForm">
...
.user-chat-button.group-button {
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
    color: white;
    border-color: #667eea;
}

.user-chat-button.group-button:hover {
    background: linear-gradient(135deg, #764ba2 0%, #667eea 100%);
}

.user-chat-button.active.group-button {
    background: linear-gradient(135deg, #764ba2 0%, #667eea 100%);
    border-color: #764ba2;
}
...
.user-chat-button.group-button .user-unread-badge {
    background: white;
    color: #667eea;
}
...
.chat-header-avatar.group-header-avatar {
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
    display: flex;
    align-items: center;
