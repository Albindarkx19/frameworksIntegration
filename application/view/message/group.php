<div class="chat-container">
    <div class="chat-sidebar">
        <div class="chat-sidebar-header">
            <h2>Admin Panel</h2>
            <a href="<?= Config::get('URL'); ?>message/index" class="btn-new-chat-small">← Back</a>
        </div>

        <div class="chat-user-buttons">
            <!-- Current active admin group -->
            <a href="<?= Config::get('URL'); ?>message/group"
               class="user-chat-button active admin-button">
                <span class="user-button-icon">⚙️</span>
                <span class="user-button-name">Admin Group</span>
            </a>
    ...
    <div class="chat-main">
        ...
        <div class="chat-header">
            <div class="chat-header-user">
                <div class="chat-header-avatar admin-header-avatar">
                    <span class="admin-icon-large">⚙️</span>
                </div>
                <div class="chat-header-info">
                    <h3>Admin Group Messages</h3>
                    <span class="chat-header-email">Broadcast messages to all administrators</span>
                </div>
            </div>
        </div>
        ...
        <div class="chat-messages-area" id="chatMessagesArea">
            ...
            <?php if (!empty($this->group_messages)) { ?>
                <?php
                $current_date = '';
                foreach ($this->group_messages as $msg) {
            ...
            <?php } else { ?>
                <div class="chat-empty-state">
                    <p>No admin messages yet. Start the conversation!</p>
                </div>
            <?php } ?>
        </div>
        ...
        <div class="chat-input-area">
            <form action="<?= Config::get('URL'); ?>message/sendGroup" method="post" id="chatForm">
                <input type="hidden" name="receiver_group" value="admin" />
                <div class="chat-input-wrapper">
                    <textarea
                        name="message_text"
                        id="messageInput"
                        placeholder="Type your message to all admins..."
                        rows="1"
                        required></textarea>
...
.user-chat-button.admin-button {
    background: #6f42c1;
    color: white;
    border-color: #6f42c1;
}

.user-chat-button.admin-button:hover {
    background: #5a32a3;
}

.user-chat-button.active.admin-button {
    background: #5a32a3;
    border-color: #5a32a3;
}
...
.chat-header-avatar.admin-header-avatar {
    background: #6f42c1;
    display: flex;
    align-items: center;
