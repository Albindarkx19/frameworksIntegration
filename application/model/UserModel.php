...
class UserModel
{
    ...
    public static function getPublicProfilesOfAllUsers()
    {
        ...
        foreach ($query->fetchAll() as $user) {
            ...
            // Store user_id first (as integer to prevent XSS filter issues)
            $safe_user_id = (int)$user->user_id;

            // Copy all other data
            $user_profile->user_name = $user->user_name;
            $user_profile->user_email = $user->user_email;
            $user_profile->user_active = $user->user_active;
            $user_profile->user_deleted = $user->user_deleted;
            $user_profile->user_has_avatar = $user->user_has_avatar;
            $user_profile->user_avatar_link = (Config::get('USE_GRAVATAR') ? AvatarModel::getGravatarLinkByEmail($user->user_email) : AvatarModel::getPublicAvatarFilePathOfUser($user->user_has_avatar, $user->user_id));

            // all elements of array passed to Filter::XSSFilter for XSS sanitation, have a look into
            // application/core/Filter.php for more info on how to use. Removes (possibly bad) JavaScript etc from
            // the user's values
            array_walk_recursive($user_profile, 'Filter::XSSFilter');

            // Restore user_id after filtering (filter may have corrupted it)
            $user_profile->user_id = $safe_user_id;

            $all_users_profiles[$safe_user_id] = $user_profile;
        }
