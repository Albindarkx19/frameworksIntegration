...
class AdminModel
{
     ...
     * @return bool
     */
    public static function updateUserdata($userId, $userName, $userEmail)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "UPDATE users SET user_name = :user_name, user_email = :user_email WHERE user_id = :user_id LIMIT 1";
        ...
        if ($query->rowCount() == 1) {
            Session::add('feedback_positive', Text::get('FEEDBACK_USERNAME_EMAIL_UPDATE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_USERNAME_EMAIL_UPDATE_FAILED'));
            return false;
        }
    }
}
